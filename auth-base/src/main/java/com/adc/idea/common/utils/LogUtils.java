...
public class LogUtils {
	...
	public static final Logger ACCESS_LOG = LoggerFactory.getLogger("access-log");
	public static final Logger LOGIN_LOG = LoggerFactory.getLogger("login-log");

	/**
	 * 用户登录日志
	 */
	public static void logLogin(SysUser user, String host) {
		StringBuilder s = new StringBuilder();
		s.append(getBlock(user.getId()));
		s.append(getBlock(user.getAccount()));
		s.append(getBlock(user.getName()));
		s.append(getBlock(host));
		s.append(getBlock(user.getLocked()));
		s.append(getBlock(user.getDeleted()));
		getLoginLog().info(s.toString());
	}
	...
	public static void logPageError(HttpServletRequest request) {
		...
		s.append(getBlock(message));
		s.append(getBlock(ServletUtils.getRemoteAddr(request)));
