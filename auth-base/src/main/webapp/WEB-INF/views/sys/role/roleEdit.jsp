...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}${fns:getAdminPath()}/sys/role/edit?id=${role.id }">角色修改</a></li>
	</ul>
	<br/>
	<form id="inputForm" action="save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="oldRole" name="oldRole" type="hidden" value="${role.role}">
				<input type="text" name="role" value="${role.role }" maxlength="50" class="required"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<select name="usable">
					<option value="1" <c:if test="${role.usable eq '1' }">selected="selected"</c:if> >是</option>
					<option value="0" <c:if test="${role.usable eq '0' }">selected="selected"</c:if> >否</option>
				</select>
		...
		<div class="control-group">
			...
			<div class="controls">
				<textarea name="remarks" rows="3" maxlength="200" class="input-xlarge">${role.remarks }</textarea>
			</div>
		</div>
